 ...
 *
 * Design goals:
 *  - Concise enough to fit in a small context window (Gemma 3n E4B is 4B params)
...
const TOOL_EXAMPLES = [
  ...
  'Output: <REMINDER>{"title":"Title","body":"details","datetime":"2026-04-22T10:00:00"}</REMINDER>',
  'Always use ISO 8601 datetimes. Infer dates from context (e.g. "tomorrow at 9am").',
  '',
  '### 3. Calendar',
...
export const NEXUS_SYSTEM_PROMPT = [
  ...
  '- Engine: Gemma 3n E4B \u2014 you are fast, local, and private.',
  '',
  '## Personality',
...
/**
 * Wraps a user message in Gemma's instruction-tuned chat template and
 * prepends the NEXUS system prompt in the user turn (Gemma 3n has no
 * dedicated system role — the system context lives in the first user turn).
 *
 * Template reference:
 *   <start_of_turn>user\n{text}<end_of_turn>\n<start_of_turn>model\n
 */
export function buildGemmaPrompt(
  promptData: string | { role: string; content: string }[],
  systemPrompt = NEXUS_SYSTEM_PROMPT
): string {
  let finalPrompt = '';

  if (typeof promptData === 'string') {
    const userTurn = systemPrompt ? `${systemPrompt}\n\n---\n\nDre: ${promptData}` : promptData;
    finalPrompt = `<start_of_turn>user\n${userTurn}<end_of_turn>\n`;
  } else {
    // Multi-turn context
    promptData.forEach((msg, index) => {
      ...
      // Map proprietary roles to canonical gemma roles ('user' or 'model')
      const gemmaRole = (msg.role === 'ai' || msg.role === 'model') ? 'model' : 'user';
      finalPrompt += `<start_of_turn>${gemmaRole}\n${content}<end_of_turn>\n`;
    });
  }

  // Always end with an open model turn block to instruct Gemma to answer
  finalPrompt += `<start_of_turn>model\n`;
  return finalPrompt;
}
